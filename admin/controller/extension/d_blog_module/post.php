...
// $url = $this->getUrl();
        ...
        foreach ($results as $result) {
            $data['posts'][] = array(
                ...
                'edit'           => $this->model_extension_d_opencart_patch_url->link('extension/d_blog_module/post/edit', '&post_id=' . $result['post_id'] . '&filter_category=' . $filter_category . $url)
            );
        }
    ...
    protected function getForm()
    {
        ...
        if (!isset($this->request->get['post_id'])) {
            ...
            $data['action'] = $this->model_extension_d_opencart_patch_url->link('extension/d_blog_module/post/edit', '&post_id=' . $this->request->get['post_id'] . '&filter_category=' . $this->request->get['filter_category'] . $url);
        }
		...
		if (isset($this->request->post['sort_order'])) {
            ...
            $data['sort_order'] = $post_info['post_sort_order'];
        } else {
            $data['sort_order'] = '0';
        }
        ...
        } elseif (!empty($post_info['url'])) {
            $data['url'] = $post_info['url'];
        } else {
            $data['url'] = '';
        }
        ...
        if (isset($this->request->post['page_image'])) {
            ...
            $data['page_image'] = $post_info['page_image'];
        } else {
            $data['page_image'] = '';
        }
		...
		if (isset($this->request->post['page_image']) && is_file(DIR_IMAGE . $this->request->post['page_image'])) {
            $data['thumb_page_image'] = $this->model_tool_image->resize($this->request->post['page_image'], 100, 100);
        } elseif (!empty($post_info) && is_file(DIR_IMAGE . $post_info['page_image'])) {
            $data['thumb_page_image'] = $this->model_tool_image->resize($post_info['page_image'], 100, 100);
        } else {
            $data['thumb_page_image'] = $this->model_tool_image->resize('no_image.png', 100, 100);
        }
    ...
    public function edit()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->response->redirect($this->model_extension_d_opencart_patch_url->link('extension/d_blog_module/post', '&filter_category=' . $this->request->get['filter_category']));
			// $this->model_extension_d_opencart_patch_url->link('extension/d_blog_module/post/edit', '&post_id=' . $this->request->get['post_id'] . '&filter_category=' . $this->request->get['filter_category'] . $url);
        }
        $this->load->model('extension/d_blog_module/author');
