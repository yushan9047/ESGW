...
class ModelExtensionDBlogModulePost extends Model
{
    ...
    public function editPost($post_id, $data)
    {
		...
		if (isset($data['page_image'])) {
            $this->db->query("UPDATE " . DB_PREFIX . "bm_post
                SET page_image = '" . $this->db->escape($data['page_image'])
                . "' WHERE post_id = '" . (int)$post_id . "'");
        }
    ...
    {
        $query = $this->db->query("SELECT DISTINCT *, p.sort_order AS post_sort_order 
    ...
    public function getPosts($data = array())
    {
        ...
        pd.language_id AS language_id, pd.title AS title, pd.tag AS tag, pd.sub_title as sub_title, p.sort_order AS sort_order 
        FROM " . DB_PREFIX . "bm_post p
        LEFT JOIN " . DB_PREFIX . "bm_post_description pd ON (p.post_id = pd.post_id)
