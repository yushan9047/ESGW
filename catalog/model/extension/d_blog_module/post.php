...
class ModelExtensionDBlogModulePost extends Model
{
    ...
    {
        $sql = "SELECT p.sort_order, p.post_id, p.image, pd.tag, pd.title, pd.meta_title, p.date_added, p.date_published, "
            . "pd.meta_description, pd.meta_keyword, pd.description, p.url, pd.sub_title, "
        ...
        $sql .= "AND p.status = 1 GROUP BY p.post_id ORDER BY p.sort_order ASC";
    ...
    {
        $sql = "SELECT p.post_id, p.sort_order, p.user_id,p.limit_access_user,p.limit_users,p.limit_access_user_group,p.limit_user_groups,p.user_id, p.image,p.image_title,p.image_alt, p.images_review, pd.tag, pd.title, pd.meta_title, p.date_added, p.date_modified, p.date_published, p.review_display, p.viewed,   "
            . "pd.meta_description, pd.meta_keyword, pd.description, pd.sub_title, p.page_image,"
    ...
    public function getNextPost($post_id, $category_id = 0)
    {
        $sql = "SELECT p.post_id, p.user_id, p.image, pd.tag, pd.title, pd.meta_title, p.date_added, p.date_modified, p.review_display, p.viewed,   "
            . "pd.meta_description, pd.meta_keyword, pd.description, pd.sub_title,"
            . "pd.short_description, COUNT(DISTINCT r.review_id) as review, ROUND(AVG(r.rating)) as rating ";
        if ($category_id) {
            $sql .= " FROM " . DB_PREFIX . "bm_post_to_category p2c";
            $sql .= " LEFT JOIN " . DB_PREFIX . "bm_post p ON (p2c.post_id = p.post_id)";
